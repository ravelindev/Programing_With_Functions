...
def main():
    ...
    draw_insect(canvas, 450, 400, 250)
    ...
    for x in range(100, 900, 50):
        x = x + 100
        draw_pine_tree(canvas, x, 100, 100)
        draw_insect(canvas, x, 100, 100)
...
def draw_insect(canvas, center_x, bottom, height):
    #draw insects
    insect_width = height / 3
    insect_height = height / 5
    left_insect = center_x - insect_width
    bottom_insect = bottom
    right_insect = center_x + insect_width
    insect_top = bottom + insect_height
    draw_oval(canvas, left_insect, bottom_insect, right_insect, insect_top, outline="salmon", fill='black')
